...
public class BeanCounterLogicTest {
	...
	public void testReset() {
		...
		int inFlightObserved = getInFlightBeanCount(logic, slotCount);
		int inSlotsObserved = getInSlotsBeanCount(logic, slotCount);
	...
	public void testAdvanceStepCoordinates() {
		...
		while (logic.advanceStep()) {
			//Check position of in-flight bean is a legal position in coords system
			int flightPosition;
			boolean legalPosition = true;
			for (int i = 0; i < slotCount; i++) {
				flightPosition = -1;
				if (logic.getInFlightBeanXPos(i) != -1) {
					flightPosition = logic.getInFlightBeanXPos(i);
					if (flightPosition > slotCount) {
						legalPosition = false;
					}
				}
			}
			assertTrue(legalPosition);
		}
	}
	...
	public void testAdvanceStepBeanCount() {
		...
		while (logic.advanceStep()) {
			...
			int inFlightObserved = getInFlightBeanCount(logic, slotCount);
			int inSlotsObserved = getInSlotsBeanCount(logic, slotCount);
			int beanCountObserved = remainingObserved + inFlightObserved + inSlotsObserved;
			
			assertEquals(failString + ". Check on bean count",
					beanCount, beanCountObserved);
		}
	}
	...
	public void testLowerHalf() {
		...
		int totalBeans = beans.length;
		
		logic.lowerHalf();
		
		int totalBeansAfterHalf = 0;
		for (int i = 0; i < slotCount; i++) {
			totalBeansAfterHalf += logic.getSlotBeanCount(i);
		}
		
		int beansShouldHave;
		if (totalBeans % 2 == 1) {
			beansShouldHave = (totalBeans + 1) / 2;
		} else {
			beansShouldHave = totalBeans / 2;
		}
		
		assertEquals(failString + ". Check on bean count",
				beansShouldHave, totalBeansAfterHalf);
	}
	...
	/**
	 * Test case for double getAverageSlotBeanCount() after void upperHalf() in skill mode.
	 * Preconditions: None.
	 * Execution steps: Call logic.reset(beans).
	 *                  Call logic.advanceStep() in a loop until it returns false (the machine terminates).
	 *                  Call logic.upperHalf();
	 * Invariants: Average after taking the upper half will always be greater or equal to total average. 
	 */
	@Test
	public void testUpperAverage() {
		if (!isLuck) {
			logic.reset(beans);
			while (logic.advanceStep()){
			}
			
			double observedAverageBeforeUpper = logic.getAverageSlotBeanCount();
			
			logic.upperHalf();
			
			double observedAverageAfterUpper = logic.getAverageSlotBeanCount();
			
			double greater = Math.abs(observedAverageAfterUpper - observedAverageBeforeUpper);
			
			assertTrue(failString, greater >= 0);
		}
	}
}
